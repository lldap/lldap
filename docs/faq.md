...
Under certain circumstances, users browsing an LLDAP service may perform automated HTTP requests to third-party [Content Delivery Networks](https://en.wikipedia.org/wiki/Content_delivery_network). That is however, dependent on the instance's specific LLDAP configuration, and any data collected in this manner is not communicated to the LLDAP project ; whether that is legal or illegal in terms of data collection depends on the server's jurisdiction.
