...
It means the backend `lldap` daemon did not find the frontend files to serve. By default, it will look for them in the `app` folder in the current working directory.
...
If you have installed LLDAP from a release tarball or using a package manager, `lldap` may be trying to look for the assets in the wrong directory. For example, if you installed the `lldap` daemon to `/usr/local/bin/lldap`, and renamed the `app` directory to `/usr/local/share/lldap`, you need to set `assets_path = "/usr/local/share/lldap"` in your configuration file `lldap_config.toml`.`
