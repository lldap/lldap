...
The frontend code is located in the `app/` folder. It consists of:

- static files: the .html files at the root, as well the `static/` subfolder
- frontend Rust code compiled to WASM: in the `src/` subfolder
- [GraphQL](https://en.wikipedia.org/wiki/GraphQL) queries in the `queries/` subfolder, to communicate with the backend

To compile the frontend WASM, run the `./app/build.sh` script. You'll need to run this after every front-end change to
update the WASM package served.

Files and folders contained in `app/` need to be readable by the `lldap` daemon. The daemon will look for them in the
configured `assets_path` folder. By default, it will try to find `./app` which will work if you run the daemon from
the project root during development.

## Backend

The backend daemon (the `lldap` CLI) can be compiled using standard `cargo` commands.
